...
class GroupMemberAdmin(admin.ModelAdmin):
    list_display = ['user', 'group', 'role', 'joined_date', 'is_active', 'created_at']
    list_filter = ['is_active', 'role', 'group', 'group__party', 'joined_date', 'created_at']
    search_fields = ['user__username', 'user__email', 'user__first_name', 'user__last_name', 'group__name']
    readonly_fields = ['created_at', 'updated_at']
    ordering = ['-joined_date']
    fieldsets = (
        ('Membership', {'fields': ('user', 'group', 'role', 'is_active')}),
